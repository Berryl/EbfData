...
def get_data_body_column(db_range: Range, df: pd.DataFrame, column_name: str) -> int:
    ...
    try:
        ...
        raise KeyError(
            f"Column '{column_name}' not found in DataFrame. Available columns: {list(df.columns)}") from None

    # Safely handle int vs. slice from the Excel COM object
    col = db_range.column
    start_col = col.start if isinstance(col, slice) else col # type: ignore[attr-defined]
